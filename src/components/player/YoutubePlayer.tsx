...
export default function YoutubePlayer(props: YoutubePlayerProps) {
  ...
  const onReady = (evt: { target: unknown }) => {
    ...
    canAutoplay.video().then(({ result }: { result: boolean }) => {
      if (!result && localPlayerRef.current) {
        (player as { mute: () => void }).mute();
      }
    });

    if (games) {
      (player as { loadVideoById: (_id: string, _time?: number) => void }).loadVideoById(
        games[part!.part - 1].video_id,
        part!.timestamp
      );
    } else {
  ...
  return (
    ...
    >
      ...
      {!isTouchDevice && !showControls && <div className="pointer-events-auto absolute inset-0 z-10 bg-transparent" />}
