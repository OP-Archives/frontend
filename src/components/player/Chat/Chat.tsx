...
export default function Chat(props: ChatProps) {
  ...
  const buildComments = useCallback(() => {
    ...
    const fetchNextComments = () => {
      ...
      archiveClient.vods
        .comments(channel!, vodId, { cursor: cursor.current ?? '' })
        .then((response) => {
          if (!response.success) {
            throw response;
          }
  ...
  }, [getCurrentTime, playerRef, vodId, isPlaying, shouldFilterMessage, channel]);
  ...
  }, [vodId, playerRef, playerState, getCurrentTime, handleScroll, isPlaying, channel]);
